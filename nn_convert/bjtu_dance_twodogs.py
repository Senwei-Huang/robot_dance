...
class BJTUDance:
    ...
    def __init__(self):
        ...
        self.scale = {"lin_vel" : 2.0,
                      ...
                      "clip_actions" : 1.2,
                      "action_scale" : 0.25}
        default_dof_pos = [[0.1, 0.8, -1.5], [-0.1, 0.8, -1.5], [0.1, 1., -1.5], [-0.1, 1., -1.5]]
        self.default_dof_pos = to_torch(default_dof_pos, device=self.device, requires_grad=False) # LF RF LH RH
        ...
        self.torque_limits = to_torch([max_effort*4], device=self.device, requires_grad=False)
    ...
    def inference_(self):
        ...
        while True:
            ...
            self.torques = self._compute_torques(self.actions_scaled).view(self.torques.shape)
            self.torques2 = self._compute_torques(self.actions2_scaled).view(self.torques2.shape)
    ...
    def PutToNet(self):
        ...
        base_quat = to_torch([x,y,z,w], dtype=torch.float32, device=self.device)
        
        base_lin_vel_w = [self.shareinfo_feed.sensor_package.body_vel[0], self.shareinfo_feed.sensor_package.body_vel[0], self.shareinfo_feed.sensor_package.body_vel[0]]
        base_ang_vel_w = [self.shareinfo_feed.sensor_package.imu_wxyz[0], self.shareinfo_feed.sensor_package.imu_wxyz[0], self.shareinfo_feed.sensor_package.imu_wxyz[0]]
        
        base_lin_vel_w = to_torch(base_lin_vel_w, dtype=torch.float32, device=self.device)
        base_ang_vel_w = to_torch(base_ang_vel_w, dtype=torch.float32, device=self.device)
        
        base_lin_vel = quat_rotate_inverse(base_quat, base_lin_vel_w)
        base_ang_vel = quat_rotate_inverse(base_quat, base_ang_vel_w)
        
        gravity_vec = to_torch([0, 0, -1], dtype=torch.float32, device=self.device)
        ...
        for i in range(4):  # LF RF LH RH
            for j in range(3):
                ...
                self.actor_state[9 + i*3 + j] = (self.dof_pos - self.default_dof_pos) * self.scale["dof_pos"]
                self.actor_state[21 + i*3 + j] = self.dof_vel * self.scale["dof_vel"]
